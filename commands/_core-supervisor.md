...
4. 所有 worker 的 phase 均为 `done` 时，整体置 `done: true`，用 `CronDelete` 清掉定时巡检 cron（CronList 找到 id 后删除），同时执行 `~/.agent-mail/registry.py unregister --session-id <你的sid> [--project-dir <project-dir>]` 从 registry 注销自己，向用户做**项目总结报告**（做了什么、质量结论、遗留风险、监工职权内的全部决策清单）。
