...
中文（本文件） | [English](DESIGN.md)

本文档记录 claude-supervisor 的设计依据、逆向考古结论、中断模型与各机制的原理。使用方法见 [README-zh.md](README-zh.md)。
...
协议长度直接关系遵循度（每加一个模式的条款都在稀释其他模式的遵循度），故模式层有硬预算：绿地模式层 39 行、rework 模式层 68 行、research 模式层 52 行、abstract 模式层 49 行、adversarial 模式层 43 行（加固后实测；fa78117 通道合法化 +1 行，此后各轮均为同行替换不增行）、core 195 行、worker 128 行。拼接产物：绿地 234 行、rework 263 行、research 247 行、abstract 244 行、adversarial 238 行（同日实测；行数表的老教训依然生效：改完必须 wc -l 再写数）。rework 的增量条款通过引用 core 既有机制（QUESTIONS 三层、Loop Guard、质询两轮上限）而非重复声明来控制体积；自包含条款（边界/错误路径/非功能三查）例外——拼接产物不含绿地层，跨模式引用会悬空（实施 CR P1-1 裁定）。research 同样以引用为主（时间盒/对账/抽查复现均引用 core 机械回放纪律），新增的 worker-facing 下发条款因 worker 读不到模式层而必须自包含。
...
**adversarial 模式（2026-09-17 起，2026-09-21 加固，第五模式）**：对抗审查任务——输入是已有产出（PR diff/方案文档/调研报告），输出是收敛审查报告。设计公理：**对抗性来自隔离，不来自 prompt 声明**——单会话切视角是上下文污染（前一视角结论锚定后一视角），prompt 里写「请批判性思考」买不到真对抗；只有独立会话+消息拓扑+落盘位置的结构性隔离才买得到。设计依据 specs/2026-09-17-adversarial-review-mode/spec.md，核心机制七条：①**phase 双侧归属**——supervisor 侧 `ingest|assign|merge` 不占 workers[].phase，worker 侧 `round1 → cross-1 →（cross-2）→ done`（对 core 的 dev-N 骨架做最重的一次覆盖，两汇合点挂 core「该 phase 明确需要汇合」例外条款）；②**round1 隔离的结构保证**——findings 只在消息体内上报、监工登记入自己分片，不落共享路径（PreToolUse 守卫已阻 worker 写他人分片），残余边界如实披露（worker 技术上可读监工分片但协议无激励）；③**匿名并集**——cross-1 下发去掉视角署名防权威跟随，每方只收他方 findings（自己的不回发防自证）；④**三态收敛**（至少两路独立一致/单方坚持经攻防/明确撤回）+ N≥3 混合多数/少数判据 + Loop Guard cross 最多 2 轮；⑤**worker 数决策表**+视角菜单+备选区（用户点名为硬约束可超推荐值）；⑥**零 commit 纪律**（审查是只读任务，覆盖 worker 默认「每 phase 立即 commit」及监工不可达时的自主兜底——中断对齐锚相应改为监工分片 findings 回显而非 git log）；⑦**减员处理**（≥2 路继续、缺方标「因减员未交叉」、<2 路降级标「对抗结构未成立」）。

**subagent 通道的实证裁决（2026-09-18，违规事故反哺）**：首场实跑监工未经用户批准自 spawn subagent 充当 worker——根因是 spec 非目标「不自动拉起 worker」没落进模式层。三轮实测（Claude Code 2.1.259 经 mc --code）定案 subagent 机制边界：同会话 SendMessage 带 transcript 复活可续、跨会话 resume 被拒、subagent 只见 spawn prompt 零泄漏——round1 隔离的结构依据由实验背书。最终设计为**双通道**：终端 worker 默认（跨天/崩溃恢复/五层防御全量），监工 subagent 为用户明示批准的备选（五约束：后台 spawn、spawn prompt 含五项下发全量原文、agent_handle 落账、中断走当轮工具错误、报告标注通道）。

**零反驳强制重审（2026-09-21 加固，五场实跑复盘产物）**：五场真机实跑 cross-1 全部零反驳零撤回——「真无懈可击」与「没人认真攻击」输出同形，无法区分，意味着模式核心主张（两轮结构保证发现被回应或撤回）从未被验证。加固规则：cross-1 事实层零反驳零撤回时（档位/归簇调解不算），监工强制抽 3-5 条高影响 findings 亲自重审攻击（亲开锚点、反向找反证、复核档位），推翻的记撤回标「监工单方裁定」；重审结论逐条即时落分片（监工崩于重审中途的断点真值）。同批加固：账本判定词钉死「登记」（APPROVE 保留给 core 阶段流转——实跑曾混记，worker 按字面读账本即汇合点绕过口子）；紧凑回显逃生口（全量回显有撑爆 subagent 上下文致死实跑案例：live worker 紧凑回显、中断丢上下文全量重发或专用分发包+指针——分发包只含该方应收内容，非隔离泄漏）；「共 N 条+END」尾标（subagent 通道 idle_notification ~4KB 截断的检测手段）；登记前去重（worker 恢复后重发旧批次不重复计汇齐数）。

**五场真机实跑的验证记录（2026-09-18 至 09-21，mall-label-manage/super-input 两仓库）**：Round 2 代码审查（70 条收敛、U51 NPE 硬故障、四方 javap 互证）；Round 3 Lion 配置审查（47 条、cap 10 盲区双路兜底）；Round 4 文档审查（首次终端通道合规、TypeFree「源码私有」连环错误坍塌——A/B 互不知情独立发现同一缺陷 L8，隔离有效性的正面实证）；Round 5 五服务完整性核查（X-1 裁决依据被反用、簇 B 系统性掉落、w1 减员重派后替补 w1b 在隔离完好下独立命中 X-1 根因——比独立同发现更强的对照实验证据）。协议韧性经受的最严酷考验：宿主机睡眠中断、上下文爆炸、网关 502 三重打击下 w5 死里逃生（分批上报+断点去重）、四路 standby 静默被 watchdog 正确豁免。五场零反驳是加固规则的直接依据（见上条）。
