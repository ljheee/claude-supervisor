...
协议长度直接关系遵循度（每加一个模式的条款都在稀释其他模式的遵循度），故模式层有硬预算：绿地模式层 39 行、rework 模式层 68 行、research 模式层 52 行、abstract 模式层 49 行、adversarial 模式层 43 行（加固后实测，此前各轮修订均为同行替换不增行）、core 195 行、worker 128 行。拼接产物：绿地 234 行、rework 263 行、research 247 行、abstract 244 行、adversarial 238 行（同日实测；行数表的老教训依然生效：改完必须 wc -l 再写数）。rework 的增量条款通过引用 core 既有机制（QUESTIONS 三层、Loop Guard、质询两轮上限）而非重复声明来控制体积；自包含条款（边界/错误路径/非功能三查）例外——拼接产物不含绿地层，跨模式引用会悬空（实施 CR P1-1 裁定）。research 同样以引用为主（时间盒/对账/抽查复现均引用 core 机械回放纪律），新增的 worker-facing 下发条款因 worker 读不到模式层而必须自包含。
...
**subagent 通道的实证裁决（2026-09-19，违规事故反哺）**：首场实跑监工未经用户批准自 spawn subagent 充当 worker——根因是 spec 非目标「不自动拉起 worker」没落进模式层。三轮实测（Claude Code 2.1.259 经 mc --code）定案 subagent 机制边界：同会话 SendMessage 带 transcript 复活可续、跨会话 resume 被拒、subagent 只见 spawn prompt 零泄漏——round1 隔离的结构依据由实验背书。最终设计为**双通道**：终端 worker 默认（跨天/崩溃恢复/五层防御全量），监工 subagent 为用户明示批准的备选（五约束：后台 spawn、spawn prompt 含五项下发全量原文、agent_handle 落账、中断走当轮工具错误、报告标注通道）。
