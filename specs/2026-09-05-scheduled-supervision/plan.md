...
1. **启动步骤**新增第 6 条（现第 6 条顺延为第 7 条）：初始化完成后创建巡检 cron。措辞要点：
   ...
   - 巡检指令自包含：① CronList 自查，任务消失（自动过期）则立即重建——例行动作非异常；② 执行巡检三步（失联判定 / pending_check 结算 / 中断补课）；③ 无事项时只输出一行"巡检正常，无待办"，不发消息不长篇报告（noop 纪律）。
...
1. **idle 活性信号**【P0-1 修订后语义】：在"中断与失联处理"章节数值 WORKER STATUS 小节之后新增小节：
