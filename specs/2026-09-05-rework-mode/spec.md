...
- 【零回归硬约束】拼接（绿地模式层 + core）与拆分前 supervisor.md 的 diff **仅允许**：模式声明节新增、章节顺序重排、【CR P0-1】三处绿地特化措辞的参数化改写（含第 7 步首阶段指令整句，展开后语义与原文等价）、【实施 CR P2-1】core 标题"阶段状态机"→"阶段状态机（dev-N 骨架）"的纯改名（语义惰性，为模式层前置阶段节的标题腾位）——除允许项外语义条款一条不减、不弱化、不改措辞（纯搬运）；
