...
#   /adversarial - initialize the current session as the adversarial-review supervisor
...
splice_command "$SRC/commands/adversarial.md" "$DEST/adversarial.md"
...
echo "  终端B（同一目录）:               /worker"
echo ""
echo "  对抗审查任务（已有产出→多视角独立审→匿名交叉攻击→收敛报告）："
echo "  终端A（任意目录）:               /adversarial 审查XX PR/方案文档 [--out <报告目录>]"
echo "  终端B（同一目录）:               /worker"
echo ""
echo "  （可开更多终端重复 /worker，多 worker 并行受监工）"
echo ""
echo "流程: 监工督促需求澄清→向你对齐→spec审查→plan审查→逐Phase开发(先自CR再受审)→总结报告（绿地）；rework 模式为 考古→安全网→改造规格→计划→逐Phase开发；research 模式为 问题定义→调研方案→逐章调研(证据五级分级+逐问对账)；abstract 模式为 材料盘点→命题草稿→对抗返工(覆盖对账+锚点抽查)；adversarial 模式为 视角菜单→多路独立审→匿名交叉攻击→收敛报告"
