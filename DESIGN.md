...
协议长度直接关系遵循度（每加一个模式的条款都在稀释其他模式的遵循度），故模式层有硬预算：绿地模式层 39 行、rework 模式层 68 行、research 模式层 52 行、abstract 模式层 49 行、core 194 行、worker 128 行（2026-09-09 双路 CR 后实测，2026-09-10 abstract 加入时复核——行数表此前五个数全过期，教训：改完必须 wc -l 再写数）。拼接产物：绿地 233 行、rework 262 行、research 246 行、abstract 243 行（同日实测）。rework 的增量条款通过引用 core 既有机制（QUESTIONS 三层、Loop Guard、质询两轮上限）而非重复声明来控制体积；自包含条款（边界/错误路径/非功能三查）例外——拼接产物不含绿地层，跨模式引用会悬空（实施 CR P1-1 裁定）。research 同样以引用为主（时间盒/对账/抽查复现均引用 core 机械回放纪律），新增的 worker-facing 下发条款因 worker 读不到模式层而必须自包含。
