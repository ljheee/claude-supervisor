...
`test_stopfailure.sh` (83 assertions), `test_watchdog.sh` (50 assertions) and `test_registry.sh` (29 assertions) are assertion-style regression tests, fully sandboxed (fake sessions dirs, fake state.json, a fake UDS server; registry tests pin a temp dir via the CLAUDE_SUPERVISOR_DIR env var; watchdog tests default to the installed path and accept a repo path as $1), keeping their temp dirs on failure for debugging and cleaning up on success. Coverage matrix:
...
**The subagent channel's empirical adjudication (2026-09-18, fed back from a violation incident)**: in the first field run the supervisor spawned its own subagents as workers without user approval — root cause: the spec's non-goal "no automatic worker launch" never made it into the mode layer. Three field experiments (Claude Code 2.1.259 via mc --code) settled the subagent mechanism's boundaries: same-session SendMessage revives with full transcript; cross-session resume is refused; a subagent sees only its spawn prompt with zero leakage — the structural basis for round1 isolation is experimentally backed. The final design is a **dual channel**: terminal workers by default (cross-day/crash recovery/full five-layer defense), supervisor-spawned subagents as a user-explicitly-approved alternative (five constraints: background spawn; spawn prompt containing the full five-item delivery verbatim; agent_handle recorded in the ledger; interruptions flow back as in-turn tool errors; the report annotates each lane's channel).
...
### 13.7 The trade-offs of the hook-ified trio

v3 upgraded two "LLM execution-drift disaster zones" into mechanical guarantees: sid acquisition (the SessionStart injector: a single stdout line `SESSION_ID <uuid> <source>`, zero judgment, zero file writes) and shard-key mistake prevention (the PreToolUse guard: Write|Edit path uuid equality-checked against stdin's sid, mismatch exits 2 with stderr printing the correct sid; direct edits to registry.json are always intercepted). The guard is a tripwire, not a wall (Bash can still bypass it — that belongs to the protocol's red-line territory); the allow path is a pure string short-circuit with zero overhead. **PostToolUse auto-register was evaluated and not adopted**: it would require parsing state.json to determine the session's role — fragile, and in semantic conflict with "registration is an explicit isolation-confirmation transaction" — so unregister/stale adjudication stays on the LLM side. Incidental gain: the source=resume injection hint became the mechanical trigger channel of the resume-recovery flow.
